...
export const Features: React.FC<FeaturesProps> = ({ t, customFeatures }) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 p-8 md:p-12 rounded-3xl relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-4">{features[0].title}</h3>
              <p className="text-slate-600 dark:text-slate-400 text-lg leading-relaxed max-w-md">{features[0].desc}</p>
            </div>

            {/* Abstract visual decor */}
            {/* Animated Graphic for Upscaling */}
            <div className="absolute top-1/2 right-8 lg:right-16 -translate-y-1/2 w-40 h-40 hidden md:flex items-center justify-center pointer-events-none">
              {/* Outer pulsing ring */}
              <div className="absolute inset-0 border-2 border-primary-500/20 rounded-3xl animate-ping opacity-75 duration-[3000ms]"></div>

              {/* Rotating dashed border */}
              <div className="absolute -inset-6 border-2 border-dashed border-primary-500/20 rounded-[2rem] animate-[spin_20s_linear_infinite]"></div>

              {/* Main container */}
              <div className="relative w-32 h-32 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/50 dark:border-white/10 flex items-center justify-center overflow-hidden">
                {/* Grid background */}
                ...
                <div className="relative z-10 transform transition-transform duration-[2000ms] animate-[pulse_3s_ease-in-out_infinite]">
              ...
              {/* Floating badges */}
              <div className="absolute -top-2 -right-2 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-xs font-bold px-2 py-1 rounded-lg shadow-lg border border-slate-100 dark:border-slate-600 animate-bounce duration-[3000ms]">
                4K
              </div>
              <div className="absolute -bottom-2 -left-2 bg-primary-600 text-white text-xs font-bold px-2 py-1 rounded-lg shadow-lg animate-bounce duration-[3000ms] delay-700">
                8K
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 p-8 rounded-3xl relative overflow-hidden group">
            ...
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">{features[1].title}</h3>
            <p className="text-slate-600 dark:text-slate-400">{features[1].desc}</p>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 p-8 rounded-3xl hover:border-primary-500/30 transition-colors">
            ...
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">{features[2].title}</h3>
            <p className="text-slate-600 dark:text-slate-400">{features[2].desc}</p>
          </div>
...
export const UseCases: React.FC<UseCasesProps> = ({ t, customCases }) => {
  const defaultCases = [
    t.useCase1,
    t.useCase2,
    t.useCase3,
  ...
  const cases = customCases || defaultCases;
...
export const FAQ: React.FC<FAQProps> = ({ t, customFaqs }) => {
